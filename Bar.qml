...
Item {
  ...
  // Clear Bar owns only the surface opacity. It deliberately leaves the
  // built-in widgets, layout, and foreground styling untouched.
  property bool clearBarWorkspaceEmpty: false
  property bool clearBarSyncQueued: false
  // Omarchy's screensaver is an overlay window. Keep the bar's layer surface
  // alive but park it outside the output while that window owns the screen.
  // This preserves the instant reveal without leaving bar icons over the
  // screensaver.
  property var clearBarScreensaverWindows: ({})
  property int clearBarScreensaverWindowCount: 0
  readonly property bool clearBarScreensaverActive: clearBarScreensaverWindowCount > 0
  readonly property bool clearBarEffectivelyHidden: barHidden || clearBarScreensaverActive
  ...
  Component.onCompleted: {
    applyBarConfig()
    clearBarSyncWorkspace()
    clearBarScreensaverProbe.running = true
    clearBarEventStream.running = true
  }
  ...
  function setRequestedTransparency(value) {
    // Omarchy 4 scopes third-party bar APIs. Clear Bar is the active bar, so
    // it can change its own surface directly without mutating shell.json or
    // invoking the stock wallpaper-contrast helper.
    requestedTransparent = false
    clearBarSyncWorkspace()
  }

  function clearBarApplyTransparency(value) {
    var nextTransparent = value === true
    clearBarWorkspaceEmpty = nextTransparent
  ...
  function clearBarSyncWorkspace() {
    if (clearBarWorkspaceProbe.running) {
      clearBarSyncQueued = true
      return
    }
    clearBarWorkspaceProbe.running = true
  }

  function clearBarSetScreensaverWindow(address, visible) {
    var key = String(address || "")
    if (!key) return

    var next = {}
    var count = 0
    for (var existing in clearBarScreensaverWindows) {
      if (existing !== key && clearBarScreensaverWindows[existing]) {
        next[existing] = true
        count++
      }
    }
    ...
    clearBarScreensaverWindows = next
    clearBarScreensaverWindowCount = count
  }

  function clearBarSetScreensaverSnapshot(clients) {
    var next = {}
    var count = 0
    for (var index = 0; index < clients.length; index++) {
      var client = clients[index] || {}
      if (String(client.class || "") !== "org.omarchy.screensaver") continue
      var address = String(client.address || "")
      if (!address || next[address]) continue
      next[address] = true
      count++
    }
    clearBarScreensaverWindows = next
  ...
  function clearBarHandleHyprlandEvent(event) {
    var raw = String(event || "")
    var separator = raw.indexOf(">>")
    var name = separator >= 0 ? raw.slice(0, separator) : raw
    var payload = separator >= 0 ? raw.slice(separator + 2) : ""

    if (name === "openwindow") {
      var open = payload.split(",")
      if (String(open[2] || "") === "org.omarchy.screensaver") {
        clearBarSetScreensaverWindow(open[0], true)
      }
    } else if (name === "closewindow") {
      var close = payload.split(",")
      if (clearBarScreensaverWindows[String(close[0] || "")]) {
        clearBarSetScreensaverWindow(close[0], false)
      }
    }
  ...
  onClearBarScreensaverActiveChanged: clearBarSyncWorkspace()
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        try {
          ...
          root.clearBarApplyTransparency(Number(workspace.windows || 0) === 0)
        } catch (e) {
          root.clearBarApplyTransparency(false)
        }
      }
    }
    onExited: {
      if (root.clearBarSyncQueued) {
        root.clearBarSyncQueued = false
        root.clearBarSyncWorkspace()
      }
    }
  }

  Process {
    id: clearBarScreensaverProbe
    command: ["hyprctl", "clients", "-j"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        try {
          root.clearBarSetScreensaverSnapshot(JSON.parse(text))
        } catch (e) {
          root.clearBarSetScreensaverSnapshot([])
        }
      }
    }
  }
  ...
  Process {
    id: clearBarEventStream
    command: ["bash", "-c", "exec socat -u \"UNIX-CONNECT:$XDG_RUNTIME_DIR/hypr/$HYPRLAND_INSTANCE_SIGNATURE/.socket2.sock\" -"]
    stdout: SplitParser {
      onRead: function(line) { root.clearBarHandleHyprlandEvent(line) }
    }
    onExited: clearBarEventReconnect.restart()
  }

  Timer {
    id: clearBarEventReconnect
    interval: 150
    repeat: false
    onTriggered: if (!clearBarEventStream.running) clearBarEventStream.running = true
  }
  ...
  component BarPanel: PanelWindow {
    ...
    exclusionMode: root.clearBarEffectivelyHidden ? ExclusionMode.Ignore : ExclusionMode.Auto
    ...
    margins {
      top: root.clearBarEffectivelyHidden && root.position === "top" ? -root.barSize : 0
      bottom: root.clearBarEffectivelyHidden && root.position === "bottom" ? -root.barSize : 0
      left: root.clearBarEffectivelyHidden && root.position === "left" ? -root.barSize : 0
      right: root.clearBarEffectivelyHidden && root.position === "right" ? -root.barSize : 0
    }
